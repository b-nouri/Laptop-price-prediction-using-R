...
maxPrice_Clean_Training <- clean6 %>% select(brand, touchscreen, screen_size , weight, ram, storage, ssd, resolution, discrete_gpu,cpu_benchmark_score,gpu_benchmark_score, max_price)

avePrice_Clean_Training <- clean6 %>% select(brand, touchscreen, screen_size , weight, ram, storage, ssd, resolution, discrete_gpu,cpu_benchmark_score,gpu_benchmark_score, ave_price)
...
##### Train the model 7 Parallel Random Forest - Max Price  <---------------BEST MODEL SO FAR
model7_max <- train(max_price ~ . , data = maxPrice_Clean_Training,
                    method = "parRF", trControl = train.control, metric = "MAE")

##### Train the model 7 Parallel Random Forest - Price Variation  <---------------BEST MODEL SO FAR
model7_avePrice <- train(ave_price ~ . , data = avePrice_Clean_Training,
                         method = "parRF", trControl = train.control, metric = "MAE")


print(min(model7_max$results$MAE+model7_avePrice$results$MAE))
